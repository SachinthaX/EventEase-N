...
export const getResaleTickets = async (req, res) => {
  try {
    const tickets = await Ticket.find({ resaleStatus: 'listed_for_resale' })
      ...
      .populate('userId', 'name email'); // Match schema

    res.status(200).json(tickets); // ✅ Must return an array
  } catch (err) {
    console.error("❌ Error fetching resale tickets:", err.message);
    res.status(500).json({ message: "Failed to load resale tickets" }); // This is what causes `.filter()` to fail
  }
};
